...
class VitisWriter(Writer):
    ...
    def write_project_cpp(self, model):
        ...
        f = open(os.path.join(filedir,'../templates/vitis/firmware/myproject.cpp'),'r')
    ...
    def write_project_header(self, model):
        ...
        filedir = os.path.dirname(os.path.abspath(__file__))
        f = open(os.path.join(filedir,'../templates/vitis/firmware/myproject.h'),'r')
    ...
    def write_defines(self, model):
        filedir = os.path.dirname(os.path.abspath(__file__))
        f = open(os.path.join(filedir,'../templates/vitis/firmware/defines.h'),'r')
    ...
    def write_parameters(self, model):
        filedir = os.path.dirname(os.path.abspath(__file__))
        f = open(os.path.join(filedir,'../templates/vitis/firmware/parameters.h'),'r')
    ...
    def write_test_bench(self, model):
        ...
        f = open(os.path.join(filedir,'../templates/vitis/myproject_test.cpp'),'r')
        ...
        for line in f.readlines():
            ...
            if 'myproject' in line:
                newline = line.replace('myproject', model.config.get_project_name())
            elif '//load top-level-function from xclbin file' in line:
                newline = line
                newline += '      cl::Kernel krnl_myproject(program, "{}");\n'.format(model.config.get_project_name())
            elif '// hls-fpga-machine-learning insert parameters declaration and then set arg' in line:
                newline = line
                ...
                setarg = indent + 'krnl_myproject.setArg(narg++, buffer_{0});\n'
                for inp in model.get_input_variables():
                    newline += indent + inp.definition_cpp() + ';\n'
                    buffer = indent + 'cl::Buffer buffer_{0}(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, {1}*{0}[0].width/8, {0}, &err);\n'
                    newline += buffer.format(inp.cppname, inp.size_cpp())
                    newline += setarg.format(inp.cppname)
                for out in model.get_output_variables():
                    # brace-init zeros the array out because we use std=c++0x
                    newline += indent + out.definition_cpp() + ';\n'
                    buffer = indent + 'cl::Buffer buffer_{0}(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, {1}*{0}[0].width/8, {0}, &err);\n'
                    newline += buffer.format(out.cppname, out.size_cpp())
                    newline += setarg.format(out.cppname)
                for in_size in input_size_vars.split(','):
                    # newline += setarg.format(in_size)
                    buffer = indent + 'cl::Buffer buffer_{0}(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, sizeof(short), &{0}, &err);\n'
                    newline += buffer.format(in_size)
                    newline += setarg.format(in_size)
                for out_size in output_size_vars.split(','):
                    # newline += setarg.format(in_size)
                    buffer = indent + 'cl::Buffer buffer_{0}(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, sizeof(short), &{0}, &err);\n'
                    newline += buffer.format(out_size)
                    newline += setarg.format(out_size)
                ...
                # size_str = indent + 'unsigned short {},{};\n'
                input_size_vars = ','.join(['buffer_size_in{}'.format(i) for i in range(1, len(model.get_input_variables()) + 1)])
                output_size_vars = ','.join(['buffer_size_out{}'.format(o) for o in range(1, len(model.get_output_variables()) + 1)])
                # newline += size_str.format(input_size_vars, output_size_vars)

                input_vars = ','.join(["buffer_" + i.cppname for i in model.get_input_variables()])
                output_vars = ','.join(["buffer_" + o.cppname for o in model.get_output_variables()])
                top_level = indent + 'q.enqueueMigrateMemObjects({{{},{},{}}} ,0);\n'.format(input_vars, input_size_vars, output_size_vars)
                newline += top_level
                newline += indent + 'q.enqueueTask(krnl_myproject);\n'
                newline += indent + 'q.enqueueMigrateMemObjects({{{0}}},CL_MIGRATE_MEM_OBJECT_HOST);\n'.format(output_vars)
                newline += indent + 'q.finish();\n'
    ...
    def write_bridge(self, model):
        ...
        filedir = os.path.dirname(os.path.abspath(__file__))
        f = open(os.path.join(filedir,'../templates/vitis/myproject_bridge.cpp'),'r')
    ...
    def write_build_script(self, model):
        ...
        f = open(os.path.join(filedir,'../templates/vitis/build_prj.tcl'),'r')
        ...
        for line in f.readlines():
            ...
            if 'set_part {xcu200-fsgd2104-2-e}' in line:
        ...
        f = open(os.path.join(filedir,'../templates/vitis/vitis_synth.tcl'),'r')
        fout = open('{}/vitis_synth.tcl'.format(model.config.get_output_dir()),'w')
        for line in f.readlines():
            line = line.replace('myproject', model.config.get_project_name())
            if '-part' in line:
        ...
        f = open(os.path.join(filedir,'../templates/vitis/build_lib.sh'),'r')
    ...
    def write_nnet_utils(self, model):
        ...
        srcpath = os.path.join(filedir,'../templates/vitis/ap_types/')
    ...
    def write_makefile(self, model):
        ...
        f = open(os.path.join(filedir,'../templates/vitis/makefile'),'r')
        fout = open('{}/makefile'.format(model.config.get_output_dir()),'w')

        for line in f.readlines():
            line = line.replace('myproject',model.config.get_project_name())
            line = line.replace('mystamp', model.config.get_config_value('Stamp'))
            if 'VPP_OPTS = --target hw' in line:
                line = 'VPP_OPTS = --target hw\n'

...
if {$opt(export)} {
  puts "***** EXPORT KERNEL *****"
  set time_start [clock clicks -milliseconds]
  export_design -format xo -output myproject.xo
  set time_end [clock clicks -milliseconds]
  report_time "EXPORT KERNEL" $time_start $time_end
}

if {$opt(vsynth)} {
  puts "***** VITIS SYNTHESIS *****"
  if {[file exist myproject_prj/solution1/syn/vhdl]} {
    set time_start [clock clicks -milliseconds]
    exec vitis -mode batch -source vitis_synth.tcl >@ stdout
    set time_end [clock clicks -milliseconds]
    report_time "VITIS SYNTHESIS" $time_start $time_end
  } else {
